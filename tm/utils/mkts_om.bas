' mkts_om v0.6.20191119 - Converts graphics
...
Dim Shared As uInteger cMaps (511, 255)
...
Dim Shared As Integer mainIndex, coloursIndex, auxIndex, cPoolIndex, tMapsIndex, cMapsIndex, defaultInk, rleIdx, metaSpritesIndex, curSpritePattern
...
Sub usage
	...
	Puts ""
	Puts "Supported modes: pals, chars, mapped, unmapped, sprites, bg, scripted, scr"
...
Sub doPals (img As Any Ptr, prefix As String, outputFn As String)
	...
	Select Case platform
		Case PLATFORM_CPC:
			...
			Print #f, "// Generated by mkts_om v0.6.20191119"
		...
		case PLATFORM_SMS
			...
			Print "// Generated by mkts_om v0.6.20191119"
			Print #f, "// Copyleft 2017-2019 by The Mojon Twins"
			Print #f, ""
			Print #f, "const unsigned char " & prefix & " [] = {"
			For i = 0 To 31
...
Sub doChars (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, max As Integer)
	...
	For y = y0 To y1 Step 8
		...
		If max <> -1 And ct = max Then Exit For
	Next y
	Puts "mkts_om v0.6.20191119 ~ Chars mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes)."
End Sub
...
Sub cpcDoSuperBuffer (img As Any Ptr)
	...
	Puts "mkts_om v0.6.20191119 ~ Superbuffer mode (12288 bytes)."
End Sub
...
Sub cpcDoScr (img As Any Ptr)
	...
	Puts "mkts_om v0.6.20191119 ~ Superbuffer mode (16384 bytes)."
...
doTmapsBreak:
	...
	Puts "mkts_om v0.6.20191119 ~ Tmaps mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes) from " & ctTmaps & " metatiles (" & (ctTmaps * i * wMeta * hMeta) & " bytes)."
End Sub

Sub doTileset (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, wMeta As Integer, hMeta As Integer, max As Integer)
	Dim As Integer x, y, x0, y0, x1, y1, ct, ctTmaps, xa, ya, i
	Dim As Integer xx, yy
	Dim As uByte attr 	' will be ignored in this Sub
	Dim As uByte pattern (31)
	...
	For y = y0 To y1 Step 8 * wMeta
		For x = x0 To x1 Step patternWidthInPixels * hMeta
			...
			For yy = 0 To hMeta - 1
				...
				For xx = 0 To wMeta - 1
					...
					Select Case platform
						Case PLATFORM_ZX, PLATFORM_CPC:
							copyFirstBytesOfArrayToMainBin pattern (), patternSize
							cMaps (cMapsIndex, cTMapIndex) = attr: cTMapIndex = cTMapIndex + 1
						Case PLATFORM_SG1000:
							copyFirstBytesOfArrayToMainBin pattern (), 8
							copyArrayToColourBinFromTo pattern (), 8, 15
							rem copyArrayToColourBinFromTo pattern (), 0, 15
			...
			cMapsIndex = cMapsIndex + 1: ctTmaps = ctTmaps + 1
			If max > 0 And ctTmaps = max Then goto doTilesetBreak
		Next x
	Next y
doTilesetBreak:

	i = 1: If platform = PLATFORM_ZX Then i = 2
	Puts "mkts_om v0.6.20191119 ~ Tileset mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes) from " & ctTmaps & " metatiles"
End Sub
...
zxDoSpritesBreak:
	...
	Puts "mkts_om v0.6.20191119 ~ Sprites mode, " & ct & " " & wMetaPixels & "x" & hMetaPixels & " sprite cells with masks extracted (" & (mainIndex) & " bytes)."
End Sub
...
cpcDoSpritesBreak:
	...
	Puts "mkts_om v0.6.20191119 ~ Sprites mode, " & ct & " " & wMetaPixels & "x" & hMetaPixels & " sprite cells extracted (" & (mainIndex - curIndex) & " bytes)."
End Sub
...
Sub zxDoBg (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer)
	...
	Puts "mkts_om v0.6.20191119 ~ BG mode, " & ct & " patterns (" & (8*ct) & " bytes). NT is " & auxIndex & " bytes."
End Sub
...
Sub cpcDoNametable (img As Any Ptr, x0 As Integer, y0 As Integer, x1 As Integer, y1 As Integer)
	...
	Puts "mkts_om v0.6.20191119 ~ Nametable mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes)."
End Sub
...
Sub smsDoRle ()
	...
	Puts "mkts_om v0.6.20191119 ~ Rle encoder, compressed to " & rleIdx*2 & " bytes."
End Sub
...
Sub generalDoRle ()
	...
	Puts "mkts_om v0.6.20191119 ~ Rle encoder, compressed to " & rleIdx & " bytes."
End Sub
...
Sub writeCmaps (fileName As String)
	Dim As Integer fOut
	Dim As Integer bytes
	Dim As Integer i, j, mult
	Dim As uByte d
	...
	Open fileName For Binary As #fOut
	bytes = 0
	For i = 0 To cMapsIndex - 1
		For j = 0 To lastwMeta * lasthMeta - 1
			bytes = bytes + 1
			d = cMaps (i, j): Put #fOut, , d
		Next j
	Next i				
	Close #fOut
	fiPuts "+ " & bytes & " bytes written"
...
Sub writeCmapsText (label As String)
	Dim As Integer bytes
	Dim As Integer i, j, mult
	Dim As uByte d
	
	fiPuts "Writing csmaps to array named " & label
	
	Print #fMapFile, "// Metatileset attributes"
	Print #fMapFile, "const unsigned char " & label & " [] = {"
	If fMapFile <> fHeaderFile Then Print #fHeaderFile, "extern const unsigned char " & label & " [];": Print #fHeaderFile, ""
	...
	While Not Eof (fIn)
		...
		Select Case Lcase (tokens (0))
			Case "mapfile"
				...
				Print #fMapFile, "// Generated by mkts_om v0.6.20191119"
			...
			Case "headerfile"
				...
				Print #fHeaderFile, "// Generated by mkts_om v0.6.20191119"
			...
			Case "output"
				...
				If tokens (1) = "tmaps" Then 
					...
					End If
				End If
				If tokens (1) = "cmaps" then writeCmaps tokens (2)
			...
			Case "write"
				...
				If tokens (1) = "tmaps" Then 
					...
					End If
				End If
				If tokens (1) = "cmaps" then writeCmapsText tokens (2)
			...
			Case "reset"
				...
				If tokens (1) = "cmaps" Then cMapsIndex = 0
			...
			Case "metatileset"
				...
				If parserFindTokenInTokens ("unmapped", tokens (), "lcase") then
					doTileset img, xc0, yc0, w, h, wMeta, hMeta, max
				Else				
