...
#define BASE_PORTRAITS 		49876
#define BASE_ITEMDESCS		50260
...
void k2t_show_time_prx_pry (void) {
	...
	for (k2t_i = 0; k2t_i < 6; ++ k2t_i) sp_PrintAtInv (pry, prx + k2t_i, gpt, *k2t_gpp ++ - 32);
}

void k2t_show_time (void) {
	// Current time age is stored in flags [127];
	// print 6 chars stored @ k2t_menu_text (6 * flags [127]);	
	pry = 0; prx = 13; gpt = 42; k2t_show_time_prx_pry ();
}
...
void k2t_show_items () {
	for (k2t_i = 0; k2t_i < STORAGE_MAX_ITEMS; ++ k2t_i) {
		_x = STORAGE_X + (k2t_i << 1) + k2t_i;
		_y = STORAGE_Y;
		_n = flags [FLAGS_OFFSET + k2t_i];
		draw_coloured_tile ();
